...
inline constexpr std::array<double, 19> kDefaultWeights = {
...
inline double clampDifficulty(double d) { return std::clamp(d, 1.0, 10.0); }
...
inline int intervalForStability(const Params& p, double stability)
{
    ...
    double interval = (stability / kFactor) *
                      (std::pow(p.requestRetention, 1.0 / kDecay) - 1.0);
...
inline double nextStabilityRecall(const Params& p, double difficulty,
                                   double stability, double retr, int grade)
{
    const double hardPenalty  = (grade == 2) ? p.w[15] : 1.0;
    const double easyBonus    = (grade == 4) ? p.w[16] : 1.0;
    const double inc = std::exp(p.w[8])
                     * (11.0 - difficulty)
                     * std::pow(stability, -p.w[9])
                     * (std::exp(p.w[10] * (1.0 - retr)) - 1.0)
                     * hardPenalty
                     * easyBonus;
...
inline double nextStabilityForget(const Params& p, double difficulty,
                                   double stability, double retr)
{
    const double sMin = p.w[11]
                      * std::pow(difficulty, -p.w[12])
...
inline Schedule schedule(const Params& p, const State& current,
                         double elapsedDays, int grade)
{
    const int g = std::clamp(grade, 1, 4);
    Schedule out;
    ...
    if (g == 1) {
        out.state.stability = nextStabilityForget(p, current.difficulty,
                                                  current.stability, retr);
        out.lapsed = true;
    } else {
        out.state.stability = nextStabilityRecall(p, current.difficulty,
