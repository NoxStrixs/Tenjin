...
Dialog {
    ...
    modal: true
    dim: true
    // Centre on the WINDOW, not the declaring item: dialogs declared inside
    // panels (e.g. Sidebar's ImportPickerDialog) otherwise centre within that
    // panel. Overlay.overlay spans the window, and the x/y math plus child
    // `parent.width` sizing all resolve against it correctly. (The earlier
    // transparent-background regression attributed to this reparent was in
