...
ThemedDialog {
    ...
    ColumnLayout {
        ...
        ActionButton {
            ...
            variant: "primary"
            onClicked: nativePickerLoader.item.open()
        }

        // Desktop-only native browser, isolated in its own file so its
        // QtQuick.Dialogs import never enters the mobile module set (it would
        // fail static module resolution on iOS, exactly like the former
        // Qt.labs.platform dependency here did).
        Loader {
            id: nativePickerLoader
            active: !Platform.isMobile
            source: "../components/DesktopMediaDialog.qml"
            Connections {
                target: nativePickerLoader.item
                enabled: nativePickerLoader.status === Loader.Ready
                function onPicked(path) {
                    root._selectedPath = path
                    root.picked(path)
                    root.accept()
                }
            }
