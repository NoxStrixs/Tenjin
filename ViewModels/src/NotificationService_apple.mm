// Apple notification backend (UserNotifications) — shared by iOS and macOS,
// which share the identical UNUserNotificationCenter API. Compiled on APPLE.
...
class NotificationServiceApple final : public NotificationService
{
public:
...
protected:
                       ...
                       const QVariantMap& /*payload*/) override
    {
        ...
        if (!center) return false;  // unbundled/unsigned build
...
{
    return std::make_unique<NotificationServiceApple>(parent);
}
